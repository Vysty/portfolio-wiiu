...
export default function WiiUPortfolio() {
  ...
  return (
    <div className={'flex h-screen w-screen overflow-hidden'}>
      {/*Left side*/}
      <div className={'w-42 h-screen flex flex-col items-center justify-start p-4'}>
      ...
      <div className={'grow h-screen flex flex-col'}>
        ...
        >
          {apps
            .map((app) => (
              <AppTile key={app.label} label={app.label} icon={app.icon} content={app.content} onOpen={handleOpen} />
            ))
            .concat(Array.from({ length: 15 - apps.length }, (_, i) => <EmptyTile key={`empty-${i}`} />))}
        </main>
        {/*Center Footer*/}
      ...
      <AnimatePresence>
        {activeContent && (
          ...
          >
            <div className="bg-white text-black rounded-lg p-8 shadow-lg" onClick={(e) => e.stopPropagation()}>
              {activeContent}
              <button className="mt-6 px-4 py-2 bg-gray-800 text-white rounded" onClick={() => setActiveContent(null)}>
                Fermer
              </button>
            </div>
          </motion.div>
